...
import { ChevronDown, Home, Info, RotateCcw, Search } from 'lucide-react';
...
interface PageProps {
    ...
    boardings: { data: Boarding[]; total: number; links: { url: string | null; label: string; active: boolean }[] };
    states: State[];
    cities?: City[];
...
export default function BoardingListing({
    boardings,
    states,
    filters,
    ...
    const resetFilters = () => {
        ...
        router.get(route('directory.boarding'), { view: view });
    };
    const fadeInUp = {
    ...
    return (
        <PublicLayout>
            <Head title={`Elite Boarding & Daycare | ${settings.site_name} Directory`} /> {/* --- CINEMATIC HERO --- */}
                        ...
                        >
                            <div className="space-y-0">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-woof-gold text-xs font-black tracking-[0.4em] uppercase">Verified Facilities</span>
                                </div>

                                <h1 className="text-4xl leading-16 font-bold tracking-wider text-woof-charcoal uppercase">
                                    Elite <span className="text-woof-gold">Boarding.</span>
                                </h1>
                            </div>

                            <p className="text-sm text-woof-charcoal/60 max-w-lg tracking-wider leading-relaxed font-medium">
                                Luxury accommodation for your most cherished companions. Discover elite boarding facilities verified for safety and comfort.
                            </p>
                        </div>
                                    ...
                                    >
                                        ...
                                        <div className="cursor-pointer space-y-0.5">
                                            ...
                                            <p className="text-woof-charcoal/50 text-[10px] font-normal">
                                                Show certified boarding facilities
                                            </p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    {boardings.data.length === 0 ? (
                        <div className="border-[#e8ded1] bg-[#fcfbf9] space-y-6 rounded-3xl border p-12 sm:p-20 text-center shadow-xs">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-woof-charcoal/60 mx-auto max-w-sm text-sm leading-relaxed font-normal">
                                    We couldn't find any professional boarding facilities matching your criteria. Try adjusting your filters.
                                </p>
                            </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                            ...
                            <div className="lg:col-span-8 space-y-6">
                                <div className="grid grid-cols-1 gap-6">
                                    {boardings.data.map((boarding, idx) => (
                                        <DirectoryCard key={boarding.id} idx={idx} item={boarding} type="boarding" view="list" />
                                    ))}
                                </div>

                                {boardings.links && boardings.links.length > 3 && (
                                    <div className="pt-8">
                                        <Pagination links={boardings.links} />
                                    </div>
                                )}
                            </div>
                        ...
                        <div>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                                {boardings.data.map((boarding, idx) => (
                                    <DirectoryCard key={boarding.id} idx={idx} item={boarding} type="boarding" view="grid" />
                                ))}
                            </div>
