...
export default function EditAppointment({ pet, appointment, appointmentTypes, statuses, vets }: PageProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('pets.appointments.update', [pet.id, appointment.id]), {
            onSuccess: () => {
    ...
    return (
        <DashboardLayout
            ...
            subtitle={`Update visit schedule for ${pet.name}`}
            actions={
        ...
        >
            <Head title={`Edit Appointment - ${pet.name}`} />
            ...
            <div className="pb-16 max-w-4xl mx-auto space-y-6">
                <form id="edit-appointment-form" onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-white rounded-3xl border border-[#e8ded1] shadow-xs p-6 sm:p-8 space-y-6">
                        ...
                        {/* Status */}
                        <div className="space-y-2">
                            <Label className="text-xs font-bold text-woof-charcoal uppercase tracking-wider">Appointment Status</Label>
