...
import { ChevronRight, Dog, Edit2, Image as ImageIcon, IndianRupee, MapPin, MoreVertical, Plus, ShieldCheck, Trash2 } from 'lucide-react';
...
interface Props {
    studServices: { data: StudService[]; links: any[]; total: number };
}

const breadcrumbs = [
    { title: 'Dashboard', href: '/dashboard' },
    { title: 'Stud Services', href: '/dashboard/stud-services' },
];

export default function StudServiceIndex({ studServices }: Props) {
    const handleDelete = (id: number) => {
        if (confirm('Are you sure you want to delete this stud service listing?')) {
            router.delete(route('dashboard.stud-services.destroy', id));
        }
    };

    const getStatusBadge = (studService: StudService) => {
        if (!studService.is_approved) {
            return (
    ...
    return (
        <DashboardLayout
            ...
            title="My Stud Services"
            subtitle="Manage your pedigree stud dog listings and breeding inquiries"
            actions={
                <Link href={route('dashboard.stud-services.create')}>
                    <Button className="bg-woof-charcoal hover:bg-woof-gold hover:text-woof-charcoal rounded-full px-6 text-xs font-bold text-white shadow-xs transition-all cursor-pointer">
                        <Plus className="mr-2 h-4 w-4" /> Add Stud Dog
                    </Button>
                </Link>
            }
        >
            <Head title="My Stud Services" />

            <div className="flex w-full flex-col gap-8 pb-16">
                {studServices.data.length > 0 ? (
                    <div className="space-y-8">
                        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            {studServices.data.map((studService: any) => (
                                <div
                                    key={studService.id}
                                ...
                                >
                                    <div>
                                        <div className="bg-[#fcfbf9] relative aspect-[4/3] w-full overflow-hidden border-b border-[#e8ded1]">
                                            {studService.featured_image_url ? (
                                                <img
                                                    src={studService.featured_image_url}
                                                    alt={studService.title}
                                            ...
                                            <div className="absolute top-4 left-4 flex flex-col gap-2 z-10">{getStatusBadge(studService)}</div>
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    >
                                                        <DropdownMenuItem asChild className="rounded-xl py-2 text-xs font-bold cursor-pointer text-woof-charcoal">
                                                            <Link
                                                                href={route('dashboard.stud-services.edit', studService.id)}
                                                        ...
                                                        <DropdownMenuItem
                                                            ...
                                                            onClick={() => handleDelete(studService.id)}
                                                        >
                                        ...
                                        <div className="space-y-4 p-6">
                                            <div className="space-y-1">
                                                ...
                                                <div className="text-xs text-woof-charcoal/60 flex items-center gap-1.5">
                                                    <MapPin className="h-3.5 w-3.5 text-woof-gold" /> {studService.city.name}, {studService.state.name}
                                                </div>
                                            </div>
                                            ...
                                            <div className="border-[#e8ded1] flex items-center justify-between border-t pt-4">
                                                <div className="space-y-0.5">
                                                    ...
                                                    <div className="text-sm font-bold text-woof-charcoal flex items-center gap-1">
                                                        {studService.fee && parseFloat(studService.fee) > 0 ? (
                                                            <>
                                                                <IndianRupee className="text-woof-gold h-3.5 w-3.5" />
                                                                <span>{parseFloat(studService.fee).toLocaleString('en-IN')}</span>
                                                            </>
                                                        ) : (
                                                            <span className="text-emerald-700 font-bold">Contact for Fee</span>
                                                        )}
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex flex-col items-end space-y-0.5">
                                                    ...
                                                    <span className="bg-amber-50 text-amber-800 border border-amber-200 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full">
                                                        Stud Service
                                                    </span>
                                                </div>
                                            </div>
                                    ...
                                    <div className="p-6 pt-0">
                                        <Link href={route('dashboard.stud-services.edit', studService.id)} className="block">
                                            <Button
                                                variant="outline"
                                            ...
                                            >
                                                Manage Stud Dog <ChevronRight className="ml-1.5 h-3.5 w-3.5" />
                                            </Button>
                                        </Link>
                                    </div>
                                </div>
                        ...
                        <div className="border-[#e8ded1] flex justify-center border-t pt-8">
                            <Pagination links={studServices.links} />
                        </div>
                    </div>
                ) : (
                    <div className="bg-white rounded-3xl border border-[#e8ded1] shadow-xs flex flex-col items-center justify-center py-20 px-6 text-center">
                        ...
                        <h3 className="text-woof-charcoal text-base font-bold">No stud dog listings yet</h3>
                        <p className="text-woof-charcoal/60 mt-1 mb-6 text-xs max-w-sm">
                            Showcase your pedigree male dog, champion bloodlines, and verified health credentials for breeding services.
                        </p>
                        <Link href={route('dashboard.stud-services.create')}>
                            <Button className="bg-woof-charcoal hover:bg-woof-gold hover:text-woof-charcoal rounded-full px-6 text-xs font-bold text-white transition-all shadow-xs cursor-pointer">
                                <Plus className="mr-2 h-4 w-4" /> Add First Stud Dog
                            </Button>
                        </Link>
                    </div>
                )}
