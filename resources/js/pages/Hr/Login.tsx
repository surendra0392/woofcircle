...
export default function HrLogin() {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        post('/hr/login', { onFinish: () => reset('password') });
    };

    return (
        <PortalAuthLayout
            title="HR Management Portal"
            description="Enter your authorized credentials to access human resources."
            portalName="Human Resources Portal"
            heading={<>Woof Circle <br /> Human Resources</>}
            subheading="Restricted access. Authenticate to manage personnel records, employee onboarding, disbursements, and departmental operations."
        >
            ...
            <form className="flex flex-col gap-6" onSubmit={submit}>
                ...
                >
                    {processing ? (
                        ...
                        <>
                            Authenticate HR Session <ArrowRight className="size-4" />
                        </>
                    )}
                </Button>
            </form>
