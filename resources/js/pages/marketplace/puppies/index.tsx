...
import CuratedListingSidebar from '@/components/public/curated-listing-sidebar';
...
interface PageProps {
    ...
    filters: {
        ...
        price_max?: string;
        search?: string;
        kci_registered?: string | boolean | number;
        is_champion?: string | boolean | number;
        orderby?: string;
        view?: 'grid' | 'list';
    };
}

export default function PuppiesListing({
    litters,
    breeds,
    states,
    featuredBreeders = [],
    ...
    const [filterData, setFilterData] = useState({
        ...
        kci_registered:
            filters.kci_registered === 'true' || filters.kci_registered === true || filters.kci_registered === '1' || filters.kci_registered === 1,
        is_champion: filters.is_champion === 'true' || filters.is_champion === true || filters.is_champion === '1' || filters.is_champion === 1,
    ...
    const resetFilters = () => {
        ...
        router.get(route('marketplace.index'), { view: view });
    };
    return (
        <PublicLayout>
                        ...
                        >
                            <div className="space-y-0">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-woof-gold text-xs font-black tracking-[0.4em] uppercase">Verified Registry</span>
                                </div>
                                <h1 className="text-4xl leading-16 font-bold tracking-wider text-woof-charcoal uppercase">
                                    Premium <span className="text-woof-gold">Puppies.</span>
                                </h1>
                            </div>
                            <p className="text-sm text-woof-charcoal/60 max-w-lg tracking-wider leading-relaxed font-medium">
                                Discover health-verified puppies from India's most prestigious ethical breeders. Every puppy is clinically documented
                                and verified.
                            </p>
                        </div>
                        <div
                        ...
                        >
                            <div className="border-[#e8ded1] focus-within:border-woof-gold/60 focus-within:shadow-md bg-white rounded-3xl sm:rounded-full p-2 pl-6 flex w-full items-center gap-4 border shadow-xs transition-all duration-300 sm:w-[620px]">
                                <div className="text-woof-charcoal/70 flex flex-1 items-center gap-3">
                                    ...
                                    <input
                                        type="text"
                                        placeholder="Search by breed, kennel, or city..."
                ...
                <section className="bg-woof-cream/30 py-6 border-b border-[#e8ded1]">
                    <div className="container-wide px-6 lg:px-12">
                        <div className="rounded-3xl border border-[#e8ded1] bg-white p-6 sm:p-8 shadow-md">
                            <div className="grid w-full grid-cols-1 items-end gap-6 lg:grid-cols-5">
                                    ...
                                    <div className="flex flex-col gap-2.5">
                                        ...
                                        >
                                            ...
                                            <div className="cursor-pointer space-y-0.5">
                                                ...
                                                <p className="text-woof-charcoal/50 text-[10px] font-normal">
                                                    Show certified champion parents
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="flex h-11 items-end">
                                    <Button
                                        onClick={() => applyFilters()}
                                        className="bg-woof-charcoal hover:bg-woof-gold hover:text-woof-charcoal h-11 w-full cursor-pointer rounded-full text-xs font-bold tracking-wider text-white uppercase shadow-md transition-all"
                                    >
                                        Update Results
                                    </Button>
                                </div>
                ...
                <div className="container-wide px-6 lg:px-12">
                    ...
                    {litters.data.length === 0 ? (
                        <div className="border-[#e8ded1] bg-[#fcfbf9] space-y-6 rounded-3xl border p-12 sm:p-20 text-center shadow-xs">
                            <div className="bg-woof-cream text-woof-gold mx-auto flex h-20 w-20 items-center justify-center rounded-2xl shadow-2xs border border-[#e8ded1]">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-woof-charcoal/60 mx-auto max-w-sm text-sm leading-relaxed font-normal">
                                    We couldn't find any puppies matching your search criteria. Please try another breed or location.
                                </p>
                            </div>
                            <Button
                                onClick={resetFilters}
                            ...
                            <div className="lg:col-span-8 space-y-6">
                                <div className="grid grid-cols-1 gap-6">
                                    {litters.data.map((litter, idx) => (
                                        <LitterCard key={litter.id} litter={litter} view="list" idx={idx} />
                                    ))}
                                </div>
                        ...
                        <div>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                {litters.data.map((litter, idx) => (
                                    <LitterCard key={litter.id} litter={litter} view="grid" idx={idx} />
                                ))}
                            </div>
                            <div className="mt-20">
                                <Pagination links={litters.links} />
