...
export default function PetEditPage({ pet, breeds = [], users = [] }: any) {
    const { data, setData, post, errors, processing } = useForm({
        ...
        is_champion: pet.is_champion || false,
        awards_count: pet.awards_count || 0,
        notes: pet.notes || '',
    });

    const [imagePreview, setImagePreview] = useState<string | null>(pet.profile_image_url || null);

    // Cleanup object URL
    useEffect(() => {
        return () => {
            if (imagePreview && imagePreview !== pet.profile_image_url && imagePreview.startsWith('blob:')) {
                URL.revokeObjectURL(imagePreview);
            }
        };
    }, [imagePreview, pet.profile_image_url]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('admin.pets.update', pet.id), {
            ...
            onError: () => {
                toast.error('Failed to update pet profile. Please check form validation errors.');
            }
        });
    };

    return (
        <AdminLayout title="Edit Pet Profile">
            <Head title={`Edit Pet Profile - ${pet.name}`} />
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-woof-charcoal">Edit Pet: {pet.name}</h2>
                    <p className="text-xs text-woof-charcoal/60">Update pet credentials, ownership assignment, and display photo</p>
                </div>
            </div>
