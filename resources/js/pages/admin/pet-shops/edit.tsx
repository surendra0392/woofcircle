...
import { ArrowLeft, Save, Info, MapPin, BookOpen, Image, UploadCloud, Link as LinkIcon, Trash2 } from 'lucide-react';
import { useState, useEffect } from 'react';

export default function PetShopEditPage({ petShop, states = [], cities = [], users = [] }: any) {
    const { data, setData, post, errors, processing } = useForm({
        user_id: petShop.user_id?.toString() || '',
        shop_name: petShop.shop_name || '',
        slug: petShop.slug || '',
        description: petShop.description || '',
        phone: petShop.phone || '',
        email: petShop.email || '',
        website: petShop.website || '',
        facebook_url: petShop.facebook_url || '',
        instagram_url: petShop.instagram_url || '',
        twitter_url: petShop.twitter_url || '',
        youtube_url: petShop.youtube_url || '',
        state_id: petShop.state_id?.toString() || '',
        city_id: petShop.city_id?.toString() || '',
    ...
    const [logoPreview, setLogoPreview] = useState<string | null>(petShop.logo_url || null);
    const [existingGallery, setExistingGallery] = useState<any[]>(petShop.gallery || []);

    // Cleanup logo preview object URL
    useEffect(() => {
        return () => {
            if (logoPreview && logoPreview !== petShop.logo_url) {
                URL.revokeObjectURL(logoPreview);
            }
        };
    }, [logoPreview, petShop.logo_url]);
    ...
    const handleDeleteGalleryImage = (imageId: number) => {
        if (confirm('Are you sure you want to delete this gallery image?')) {
            router.delete(route('admin.pet-shops.gallery.destroy', imageId), {
                onSuccess: () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('admin.pet-shops.update', petShop.id), {
            forceFormData: true,
            onSuccess: () => {
                toast.success('Pet Shop updated successfully.');
            },
            onError: () => {
                toast.error('Failed to update Pet Shop. Please check form validation errors.');
            }
        });
    };
    ...
    return (
        <AdminLayout title="Edit Pet Shop">
            <Head title={`Edit Pet Shop - ${petShop.shop_name}`} />
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-woof-charcoal">Edit Pet Shop: {petShop.shop_name}</h2>
                    <p className="text-xs text-woof-charcoal/60">Update retail profile, location, contact, and storefront assets</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 mt-6" encType="multipart/form-data">
                                        ...
                                        <div className="border border-[#e8ded1] p-3 rounded-2xl bg-[#fcfbf9] space-y-2">
                                            ...
                                            <div className="grid grid-cols-4 gap-2">
                                                {existingGallery.map((img) => (
                                                    <div key={img.id} className="relative aspect-square border border-[#e8ded1] rounded-xl overflow-hidden group">
                                                        <img src={img.image_url} alt="Gallery item" className="h-full w-full object-cover" />
                                                        <button
                                                            type="button"
                ...
                <div className="flex justify-end gap-3 border-t border-[#e8ded1] pt-6">
                    ...
                    >
                        <Save className="h-4 w-4" /> Update Shop
                    </Button>
                </div>
            </form>
        </AdminLayout>
