...
export default function LitterCreatePage({ breeds = [], users = [], profiles = [], states = [], cities = [] }: any) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.litters.store'), {
            forceFormData: true,
        });
    };

    return (
        <AdminLayout title="Add Litter">
            <Head title="Add Litter" />
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-woof-charcoal">Add Litter</h2>
                    <p className="text-xs text-woof-charcoal/60">Configure litter details, pricing, lineage, and gallery assets</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 mt-6" encType="multipart/form-data">
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="flex flex-col gap-4">
                                    {data.images && data.images.length > 0 ? (
                                        ...
                                        <div className="h-28 border-2 border-dashed border-[#e8ded1] hover:border-woof-gold transition-colors p-4 bg-[#fcfbf9] rounded-2xl flex flex-col items-center justify-center text-center relative cursor-pointer">
                                            <UploadCloud className="h-6 w-6 text-woof-charcoal/40 mb-1" />
                                            <span className="text-xs font-bold text-woof-charcoal">Upload Litter Gallery Images</span>
                                            <input
                                                type="file"
                                                multiple
                                                accept="image/*"
                ...
                <div className="flex justify-end pt-6 border-t border-[#e8ded1] gap-3">
                    ...
                    >
                        <Save className="h-4 w-4" /> {processing ? 'Saving...' : 'Save Litter'}
                    </Button>
                </div>
            </form>
        </AdminLayout>
