...
export default function BreederEditPage({ breeder, states, cities, availableUsers }: any) {
    const { data, setData, post, errors, processing } = useForm({
        ...
        state_id: breeder.state_id || '',
        city_id: breeder.city_id || '',
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.breeders.update', breeder.id), {
            forceFormData: true,
        });
    };

    return (
        <AdminLayout title="Edit Breeder">
            <Head title={`Edit Breeder - ${breeder.kennel_name}`} />
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-woof-charcoal">Edit Breeder: {breeder.kennel_name}</h2>
                    <p className="text-xs text-woof-charcoal/60">Update kennel profile, contact details, and certifications</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 mt-6" encType="multipart/form-data">
                    ...
                    <div className="lg:col-span-2 bg-white border border-[#e8ded1] p-6 rounded-3xl shadow-xs space-y-4">
                        ...
                        <div className="space-y-1.5">
                            ...
                            <SearchableSelect 
                                options={[
                                    { value: '', label: 'None' },
                                    ...(breeder?.user && !availableUsers?.find((u:any) => u.id === breeder.user.id) 
                                        ? [{ value: breeder.user.id.toString(), label: `${breeder.user.name} (${breeder.user.email}) - ${breeder.user.role?.name || 'No Role'}` }] 
                                        : []),
                                ...
                                <div className="flex flex-col gap-4">
                                    {data.logo ? (
                                        <div className="h-28 w-28 border border-[#e8ded1] rounded-2xl bg-[#fcfbf9] flex items-center justify-center relative overflow-hidden group shadow-2xs">
                                            <img src={URL.createObjectURL(data.logo)} alt="New Logo Preview" className="h-full w-full object-cover" />
                                        ...
                                        <div className="h-28 w-28 border border-[#e8ded1] rounded-2xl bg-[#fcfbf9] flex items-center justify-center relative overflow-hidden group shadow-2xs">
                                            <img src={breeder.logo_url} alt="Current Logo" className="h-full w-full object-cover" />
                                            <div className="absolute inset-0 bg-black/45 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                                <span className="text-[10px] text-white font-bold uppercase text-center px-2">Replace</span>
                                            </div>
                                            <input
                                                type="file"
                                                accept="image/*"
                                ...
                                <div className="flex flex-col gap-4">
                                    {data.gallery && data.gallery.length > 0 ? (
                                        <div className="border border-[#e8ded1] p-3 rounded-2xl bg-[#fcfbf9] space-y-3 shadow-2xs">
                                            <p className="text-[10px] font-bold text-woof-charcoal/60">New Gallery Preview (will replace current)</p>
                                        ...
                                        <div className="border border-[#e8ded1] p-3 rounded-2xl bg-[#fcfbf9] space-y-3 shadow-2xs">
                                            <p className="text-[10px] font-bold text-woof-charcoal">Current Gallery</p>
                                            <div className="flex flex-wrap gap-2">
                                                {breeder.gallery.map((img: any, idx: number) => (
                                                    <img key={idx} src={img.image_url || img.url || img} alt={`Gallery ${idx}`} className="h-14 w-14 object-cover rounded-xl border border-[#e8ded1]" />
                                                ))}
                                            </div>
                                            <div className="relative border-2 border-dashed border-[#e8ded1] hover:border-woof-gold transition-colors p-3 bg-white rounded-xl flex flex-col items-center justify-center text-center cursor-pointer">
                                                <UploadCloud className="h-5 w-5 text-woof-charcoal/40 mb-1" />
                                                <span className="text-[11px] font-bold text-woof-charcoal">Upload New Gallery Photos to Replace</span>
                                                <input
                                                    type="file"
                                                    multiple
                                                    accept="image/*"
                ...
                <div className="flex justify-end pt-6 border-t border-[#e8ded1] gap-3">
                    ...
                    >
                        <Save className="h-4 w-4" /> {processing ? 'Updating...' : 'Update Breeder'}
                    </Button>
                </div>
            </form>
        </AdminLayout>
