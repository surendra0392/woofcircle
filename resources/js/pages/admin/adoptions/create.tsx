...
import { ArrowLeft, Save, Info, Heart, Globe, Image as ImageIcon, Sparkles, Award } from 'lucide-react';
...
export default function AdoptionCreatePage({ breeds = [], users = [], profiles = [], states = [], cities = [] }: PageProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('admin.adoptions.store'), {
            onSuccess: () => {
                toast.success('Adoption listing created successfully.');
            },
            onError: () => {
                toast.error('Failed to create adoption listing. Please check form validation errors.');
            }
        });
    };
    ...
    const filteredProfiles = profiles.filter(p => !data.user_id || p.user_id?.toString() === data.user_id);

    return (
        <AdminLayout title="Add Pet Adoption">
            <Head title="Add Pet Adoption - Admin" />
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-woof-charcoal">Create Adoption Listing</h2>
                    <p className="text-xs text-woof-charcoal/60">Configure rescued pet details, adoption fee, location, and photos</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 mt-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="space-y-1.5">
                        ...
                        <p className="text-xs text-woof-charcoal/60 leading-relaxed pl-12">
                            Upload a primary featured thumbnail and multiple photo gallery options.
                        </p>
                    </div>
                    ...
                    <div className="lg:col-span-2 bg-white border border-[#e8ded1] p-6 rounded-3xl shadow-xs space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1.5">
                                <Label htmlFor="images" className="text-xs font-bold text-woof-charcoal">Additional Gallery Photos</Label>
                                <Input 
                                    id="images" 
                                    type="file"
                                    multiple
