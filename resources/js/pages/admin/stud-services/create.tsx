...
import { ArrowLeft, Save, Info, MapPin, BookOpen, Image, UploadCloud, ShieldCheck, Tag, IndianRupee } from 'lucide-react';
import { useState, useEffect } from 'react';

export default function StudServiceCreatePage({ breeds = [], users = [], profiles = [], states = [], cities = [] }: any) {
    const { data, setData, post, errors, processing } = useForm({
        ...
        fee: '',
        age: '',
        kci_registered: false as boolean,
        sire_name: '',
        dam_name: '',
        state_id: '',
        city_id: '',
        status: 'draft',
        is_negotiable: false as boolean,
        is_vaccinated: false as boolean,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.stud-services.store'), {
            forceFormData: true,
            onSuccess: () => {
                toast.success('Stud service listing created successfully.');
            },
            onError: () => {
                toast.error('Failed to create stud service listing. Please check form validation errors.');
            }
        });
    };

    return (
        <AdminLayout title="Add Stud Service">
            <Head title="Add Stud Service - Admin" />
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-woof-charcoal">Add Stud Service</h2>
                    <p className="text-xs text-woof-charcoal/60">Register a new stud mating listing with pedigree and commercial terms</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 mt-6" encType="multipart/form-data">
                    ...
                    <div className="lg:col-span-2 bg-white border border-[#e8ded1] p-6 rounded-3xl shadow-xs space-y-4">
                        ...
                        {data.kci_registered && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 border border-dashed border-[#e8ded1] rounded-2xl bg-[#fcfbf9]">
                                ...
                                <div className="space-y-1.5 col-span-1 md:col-span-2">
                                    <Label className="text-xs font-bold text-woof-charcoal">KCI Registration Documents</Label>
                                    <div className="flex flex-col gap-3">
                                        {data.kci_images && data.kci_images.length > 0 ? (
                                            <div className="border border-[#e8ded1] p-3 rounded-2xl bg-white space-y-3">
                                                <div className="flex flex-wrap gap-2">
                                                    {data.kci_images.map((file: File, idx: number) => (
                                                        <img key={idx} src={URL.createObjectURL(file)} alt={`KCI Document ${idx}`} className="h-16 w-16 object-cover rounded-xl border border-[#e8ded1]" />
                                                    ))}
                                                </div>
                                                <div className="flex justify-end">
                                                    <button 
                                                        type="button"
                                                        onClick={() => setData('kci_images', [])}
                                            ...
                                            <div className="h-24 border-2 border-dashed border-[#e8ded1] hover:border-woof-gold transition-colors p-4 bg-white rounded-2xl flex flex-col items-center justify-center text-center relative cursor-pointer">
                                                <UploadCloud className="h-6 w-6 text-woof-charcoal/40 mb-1" />
                                                <span className="text-xs font-bold text-woof-charcoal">Upload KCI Pedigree Certificates</span>
                                                <input
                                                    type="file"
                                                    multiple
                                                    accept="image/*"
                                                    onChange={e => setData('kci_images', Array.from(e.target.files || []))}
                                                    className="absolute inset-0 opacity-0 cursor-pointer"
                                                />
                                            </div>
                                        )}
                                    </div>
                                    {errors.kci_images && <p className="text-xs text-rose-500">{errors.kci_images}</p>}
                                </div>
                            </div>
                        )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="space-y-1.5">
                        ...
                        <p className="text-xs text-woof-charcoal/60 leading-relaxed pl-12">
                            Upload a featured photo and add multi-image uploads to build out the stud dog gallery.
                        </p>
                    </div>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="flex flex-col gap-3">
                                    {data.images && data.images.length > 0 ? (
                                        <div className="border border-[#e8ded1] p-3 rounded-2xl bg-[#fcfbf9] space-y-3">
                                            <div className="flex flex-wrap gap-2">
                                                {data.images.map((file: File, idx: number) => (
                                                    <img key={idx} src={URL.createObjectURL(file)} alt={`Gallery Preview ${idx}`} className="h-16 w-16 object-cover rounded-xl border border-[#e8ded1]" />
                                                ))}
                                            </div>
                                            <div className="flex justify-end">
                                                <button 
                                        ...
                                        <div className="h-32 border-2 border-dashed border-[#e8ded1] hover:border-woof-gold transition-colors p-4 bg-[#fcfbf9] rounded-2xl flex flex-col items-center justify-center text-center relative cursor-pointer">
                                            <UploadCloud className="h-7 w-7 text-woof-charcoal/40 mb-1" />
                                            <span className="text-xs font-bold text-woof-charcoal">Upload Gallery Images</span>
                                            <input
                                                type="file"
                                                multiple
                                                accept="image/*"
                ...
                <div className="flex justify-end gap-3 border-t border-[#e8ded1] pt-6">
                    ...
                    >
                        <Save className="h-4 w-4" /> Save Stud Service
                    </Button>
                </div>
            </form>
        </AdminLayout>
