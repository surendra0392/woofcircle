...
export default function MedicalRecordCreatePage({ pets = [], vets = [], recordTypes = [] }: PageProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('admin.medical-records.store'), {
            onSuccess: () => {
                toast.success('Medical record registered successfully.');
            },
            onError: () => {
                toast.error('Failed to register medical record. Please check validation errors.');
            }
        });
    };

    return (
        <AdminLayout title="Add Medical Record">
            <Head title="Register Medical Record - Admin" />
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-woof-charcoal">Register Medical Record</h2>
                    <p className="text-xs text-woof-charcoal/60">Log veterinary diagnosis, clinical procedures, and prescriptions</p>
                </div>
            </div>
