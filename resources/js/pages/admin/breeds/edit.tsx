...
export default function EditBreed({ breed, groups = [] }: any) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        other_names: breed.other_names || '',
        naming: breed.naming || '',
        variants: breed.variants || '',
        ...
        male_height: breed.male_height || '',
        female_height: breed.female_height || '',
        male_weight: breed.male_weight || '',
        female_weight: breed.female_weight || '',
        ...
        is_indian: breed.is_indian,
        image_file: null as File | null,
    });

    const [imagePreview, setImagePreview] = useState<string | null>(
        breed.image ? `/storage/${breed.image}` : null
    );

    // Cleanup object URL
    useEffect(() => {
        return () => {
            if (data.image_file && imagePreview && imagePreview.startsWith('blob:')) {
                URL.revokeObjectURL(imagePreview);
            }
        };
    }, [imagePreview, data.image_file]);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('admin.breeds.update', breed.id), {
            forceFormData: true,
        });
    };

    return (
        <AdminLayout title="Edit Breed">
            <Head title={`Edit ${breed.name}`} />
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-woof-charcoal">Edit Breed: {breed.name}</h2>
                    <p className="text-xs text-woof-charcoal/60">Update canine breed profile, characteristics, and media</p>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 mt-6">
                ...
                <div className="flex justify-end pt-6 border-t border-[#e8ded1] gap-3">
                    ...
                    >
                        <Save className="h-4 w-4" /> {processing ? 'Saving...' : 'Update Breed'}
                    </Button>
                </div>
            </form>
        </AdminLayout>
