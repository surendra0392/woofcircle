...
import { ArrowLeft, Save, Info, Building2, Globe, FileText, Image as ImageIcon, Trash2, GraduationCap } from 'lucide-react';
...
export default function TrainerEditPage({ trainer, states = [], cities = [], availableUsers = [], allSpecializations = [] }: PageProps) {
    const { data, setData, post, errors, processing } = useForm({
        user_id: trainer.user_id?.toString() || '',
        name: trainer.name || '',
        description: trainer.description || '',
        phone: trainer.phone || '',
        email: trainer.email || '',
        experience_years: trainer.experience_years?.toString() || '',
        state_id: trainer.state_id?.toString() || '',
        city_id: trainer.city_id?.toString() || '',
    ...
    const handleDeleteGalleryImage = (imageId: number) => {
        if (confirm('Are you sure you want to remove this gallery image?')) {
            router.delete(route('admin.trainers.gallery.destroy', imageId), {
                onSuccess: () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        post(route('admin.trainers.update', trainer.id), {
            ...
            onError: () => {
                toast.error('Failed to update trainer profile. Please check form validation errors.');
            }
        });
    };
    ...
    return (
        <AdminLayout title="Edit Dog Trainer">
            <Head title={`Edit Trainer - ${trainer.name}`} />
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h2 className="text-2xl font-bold tracking-tight text-woof-charcoal">Edit Trainer: {trainer.name}</h2>
                    <p className="text-xs text-woof-charcoal/60">Update trainer credentials, active specializations, and portfolio gallery</p>
                </div>
            </div>
                    ...
                    <div className="lg:col-span-2 bg-white border border-[#e8ded1] p-6 rounded-3xl shadow-xs space-y-4">
                        <div className="space-y-1.5">
                            ...
                            >
                                ...
                                {trainer.user && !availableUsers.some(u => u.id === trainer.user?.id) && (
                                    <option value={trainer.user.id.toString()}>{trainer.user.name} ({trainer.user.email})</option>
                                )}
                    ...
                    <div className="lg:col-span-2 bg-white border border-[#e8ded1] p-6 rounded-3xl shadow-xs space-y-6">
                        ...
                        {trainer.gallery && trainer.gallery.length > 0 && (
                            <div className="border-t border-[#e8ded1] pt-4">
                                <Label className="text-xs font-bold text-woof-charcoal block mb-3">Currently Active Gallery ({trainer.gallery.length})</Label>
                                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
                                    {trainer.gallery.map((img) => (
